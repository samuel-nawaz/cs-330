...
int main()
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// load image, create texture and generate mipmaps
	data = stbi_load("images/pinkMarble.jpg", &width, &height, &nrChannels, 0);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		glGenerateMipmap(GL_TEXTURE_2D);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// load image, create texture and generate mipmaps
	data = stbi_load("images/smiley.png", &width, &height, &nrChannels, 0);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		glGenerateMipmap(GL_TEXTURE_2D);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// load image, create texture and generate mipmaps
	data = stbi_load("images/PIplastic.jpg", &width, &height, &nrChannels, 0);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		glGenerateMipmap(GL_TEXTURE_2D);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// load image, create texture and generate mipmaps
	data = stbi_load("images/siding.jpg", &width, &height, &nrChannels, 0);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		glGenerateMipmap(GL_TEXTURE_2D);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// render the ground as a plane
		glBindTexture(GL_TEXTURE_2D, texture1);
		model = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first		
		model = glm::scale(model, glm::vec3(10.0f, 10.0f, 10.0f));
		ourShader.setMat4("model", model);
		static_meshes_3D::Plane plane;
		plane.render();

		// render the ball as a sphere
		glBindTexture(GL_TEXTURE_2D, texture2);
		model = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first		
		...
		ourShader.setMat4("model", model);
		static_meshes_3D::Cylinder cylinder(0.4f, 30, 1);
		cylinder.render();

		// render the bottle cap as a torus
		glBindTexture(GL_TEXTURE_2D, texture4);
		model = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first		
		...
		// render the wall decoration as a cube
		glBindTexture(GL_TEXTURE_2D, texture5);
		glBindVertexArray(VAO);
		model = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first		
		model = glm::translate(model, glm::vec3(1.5f, 1.0f, 0.0f));
